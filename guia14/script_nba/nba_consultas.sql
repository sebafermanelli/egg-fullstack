...
-- 10
SELECT 
    j.nombre, jugador, SUM(puntos_por_partido)
FROM
    estadisticas e
        INNER JOIN
    jugadores j ON e.jugador = j.codigo
GROUP BY e.jugador
ORDER BY e.jugador ASC;

-- 11
SELECT 
    e.nombre, COUNT(j.nombre_equipo)
FROM
    jugadores j
        INNER JOIN
    equipos e ON j.nombre_equipo = e.nombre
...
FROM
    partidos
ORDER BY ABS(diferencia_puntos) DESC
LIMIT 10;
