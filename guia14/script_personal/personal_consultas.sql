...
-- 12
SELECT 
    nombre, (sal_emp + comision_emp + 500) AS total_pagar
FROM
    empleados
WHERE
    id_depto = 3000
...
SELECT 
    MAX(sal_emp),
    MIN(sal_emp),
    (MAX(sal_emp) - MIN(sal_emp)) AS diferencia_max_min
FROM
    empleados;

-- 24
SELECT 
    AVG(sal_emp) AS salario_promedio, id_depto
FROM
    empleados
GROUP BY id_depto;
